...
public class Dispatcher {
	...
	public ActionResult service(BeatContext beat) throws Exception {
		Action action = findAction(beat);
		if(action == null){
			throw new HTTP404Exception();
		}else{
			ActionResult result = InterceptorHandler.excutePreInterceptor(action);
			if(result!=null){
				return result;
			}
			result = action.invoke();
			InterceptorHandler.excutePostInterceptor(action, result);
			return result;
		}		
	}
	...
	private Action findAction(BeatContext beat) {
		...
		Map<String, List<MethodAction>> ministyActions = ActionInit.getMinistyActions();
